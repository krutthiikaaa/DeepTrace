...
def _get_glint_and_centroid(crop: np.ndarray) -> Optional[Tuple[Tuple[float, float], np.ndarray, float]]:
...
def analyze(image: np.ndarray) -> SignalResult:
    ...
    try:
        ...
        with mp_face_mesh.FaceMesh(
            ...
            iod = math.hypot(l_eye_inner[0] - r_eye_inner[0], l_eye_inner[1] - r_eye_inner[1])
            if iod < 30:
                return SignalResult(
                    signal_name="pupil_specular_mismatch",
                    score=0.0,
            ...
            l_iris_r = math.hypot(get_px(469)[0] - l_iris_c[0], get_px(469)[1] - l_iris_c[1])
            r_iris_r = math.hypot(get_px(474)[0] - r_iris_c[0], get_px(474)[1] - r_iris_c[1])
            ...
            def crop_iris(center: Tuple[float, float], radius: float) -> Tuple[Optional[np.ndarray], Tuple[int, int]]:
            ...
            v_l = np.array([l_glint_global[0] - l_iris_c[0], l_glint_global[1] - l_iris_c[1]]) / l_iris_r
            v_r = np.array([r_glint_global[0] - r_iris_c[0], r_glint_global[1] - r_iris_c[1]]) / r_iris_r
            
            d_pos = float(np.linalg.norm(v_l - v_r))
            d_color = float(np.linalg.norm(l_color - r_color) / (255.0 * math.sqrt(3)))
            
            raw_score = 0.7 * min(1.0, d_pos / 0.4) + 0.3 * min(1.0, d_color / 0.3)
            final_score = float(np.clip(raw_score, 0.0, 1.0))
            
            out_size = 150
            l_viz = cv2.resize(l_crop, (out_size, out_size), interpolation=cv2.INTER_CUBIC)
            r_viz = cv2.resize(r_crop, (out_size, out_size), interpolation=cv2.INTER_CUBIC)
            
            def draw_markers(viz: np.ndarray, iris_c: Tuple[float, float], glint_c: Tuple[float, float], offset: Tuple[int, int], crop_shape: Tuple[int, ...]) -> np.ndarray:
                ...
                cv2.drawMarker(viz, (int(ic_x), int(ic_y)), (255, 255, 0), cv2.MARKER_CROSS, 10, 2)
                cv2.circle(viz, (int(gc_x), int(gc_y)), 5, (0, 0, 255), 2)
                return viz
                
            l_viz = draw_markers(l_viz, l_iris_c, l_glint_global, l_offset, l_crop.shape)
            r_viz = draw_markers(r_viz, r_iris_c, r_glint_global, r_offset, r_crop.shape)
            ...
            cv2.putText(banner, text, (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
